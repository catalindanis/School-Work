...
//declararea fisierului de citire al datelor de intrare
std::ifstream read("Nume_Pr3_in.txt");

//declararea variabilelor global, folosite in program
...
//readFromFile() -> citeste variabila nume, din fisier;
//-in cazul in care fisierul este gol, afiseaza un mesaj corespunzator,
//si opreste executia programului, returnand valoarea -1;
void readFromFile(){
    read>>input;
    if(input.empty()){
...
void processInput(){
    int n;
    if(!(std::cin>>n)){
        std::cout<<invalidInputExceptionMessage;
    }
    switch(n){
        case 0:
            exitRequested = true;
